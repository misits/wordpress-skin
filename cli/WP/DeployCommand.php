...
class DeployCommand {
     ...
     *
     *     wp wpskin deploy
     *     wp wpskin deploy --output=../production-build
     *     wp wpskin deploy --skip-build --clean
     *
     * @when after_wp_load
     */
    public function __invoke($args, $assoc_args) {
        ...
        if (!file_exists($theme_root . '/lib/wp-skin/bootstrap.php')) {
            \WP_CLI::error('WP-Skin not found in current theme. Make sure you\'re running this from a theme with WP-Skin installed.');
        }
        ...
        try {
            $this->buildForProduction($theme_root, $output_dir, $skip_build, $clean);
     ...
     */
    private function buildForProduction($theme_root, $output_dir, $skip_build, $clean) {
    ...
    private function buildAssets($resources_dir) {
        if (!file_exists($resources_dir . '/package.json')) {
            \WP_CLI::line('[WARNING] No package.json found, skipping asset build');
            return;
        }
    ...
    private function copyThemeFiles($theme_root, $output_dir) {
        ...
        foreach ($iterator as $item) {
            $relativePath = str_replace($theme_root . '/', '', $item->getPathname());
            
            // Skip if matches gitignore patterns
            if ($this->matchesGitignorePattern($relativePath, $gitignore_patterns)) {
                continue;
            }
